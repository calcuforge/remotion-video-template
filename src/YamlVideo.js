  ...
  return (
    <AbsoluteFill style={{ backgroundColor: themeProps.backgroundColor }}>
      <Scale4K orientation={themeProps.orientation} scaleFactor={themeProps.scaleFactor}>
        ...
        {/* Background music — a full-composition track. loop makes it repeat to
            fill the video; with loop=false it plays once then goes silent. */}
        {config.bgm?.audio && (
          <Audio
            src={resolveAssetSrc(config.bgm.audio)}
            volume={config.bgm.volume ?? 0.1}
