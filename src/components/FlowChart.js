...
/**
 * ArrowConnector — animated SVG arrow connecting two flow steps.
 * `vertical` flips orientation between horizontal (left-to-right) and vertical (top-to-bottom).
 */
const ArrowConnector = ({ vertical, color, enabled, delay }) => {
  ...
  return (
    <svg width={w} height={h} viewBox={`0 0 ${w} ${h}`} style={{ overflow: "visible" }}>
      <path d={linePath} fill="none" stroke={color} strokeWidth={2.5} strokeOpacity={0.5}
...
const FlowStep = ({ index, step, props, delay, vertical, iconAnim, gapSize, isLast }) => {
    ...
    <React.Fragment>
      ...
      {!isLast && (
        <div style={{
          ...
          <ArrowConnector vertical={vertical} color={props.primaryColor} enabled={props.enableAnimations} delay={staggerDelay(index, delay + 5, 10)} />
        </div>
      )}
    </React.Fragment>
  );
...
/**
 * FlowChart — horizontal (or vertical) sequence of labeled steps with animated arrows.
 * Each step: { label, description?, icon? }.
 */
