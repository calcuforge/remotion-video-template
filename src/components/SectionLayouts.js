...
import { MovingGradient, FloatingShapes, GlowOrb, GridPattern, AccentLine } from "./AnimatedBackground.js";
import { Icon } from "./Icon.js";

/**
 * SectionLayouts — higher-level section layouts combining backgrounds,
 * animations, and content placement. Pick one per section.
 *
 * Each layout is theme-aware (uses props.primaryColor etc.) and
 * orientation-aware (adapts for 16:9 / 9:16).
 */

// 1. SplitLayout — content left, visual right (stacked on vertical)
    ...
    <AbsoluteFill style={{ backgroundColor: props.backgroundColor }}>
      ...
      <div style={{
        ...
        <div style={{
          ...
          <h2 style={{
            fontSize: v ? 64 : 72, fontWeight: 800,
            color: props.primaryColor, lineHeight: 1.15, marginBottom: 20,
          }}>
            {title}
          </h2>
          <p style={{
    ...
    <AbsoluteFill style={{ backgroundColor: props.backgroundColor }}>
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "baseline", gap: 8 }}>
          <span style={{
            fontSize: 160, fontWeight: 900, color: props.primaryColor,
        ...
        <p style={{
          fontSize: 40, fontWeight: 700, color: props.textColor, marginTop: 16,
        }}>
          {label}
        </p>
        {description && (
          <p style={{
            fontSize: 28, color: props.textColor, opacity: 0.5,
            marginTop: 12, maxWidth: 700, textAlign: "center",
          }}>
            {description}
          </p>
        )}
...
const ZigzagCard = ({ index, item, color, props, alignRight }) => {
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: props.backgroundColor }}>
      ...
      <div style={{
        ...
        display: "flex", flexDirection: "column",
        alignItems: "center", justifyContent: "center",
        padding: "60px 100px", textAlign: "center",
        opacity: anim.opacity,
        transform: `translateY(${anim.translateY}px) scale(${anim.scale})`,
      }}>
        {icon && <Icon name={icon} size={72} color={props.primaryColor} animate="entrance" />}
        <h2 style={{
          fontSize: 72, fontWeight: 800, color: props.primaryColor,
          marginTop: icon ? 24 : 0, marginBottom: 24, lineHeight: 1.2, maxWidth: 1200,
        }}>
          {title}
        </h2>
        <p style={{
          fontSize: 32, color: props.textColor, lineHeight: 1.7,
          opacity: 0.7, maxWidth: 1000,
        }}>
          {body}
        </p>
      </div>
...
const MetricCard = ({ index, metric, color, props, vertical }) => {
  ...
  return (
    <div style={{
      ...
      <div style={{
        fontSize: vertical ? 48 : 56, fontWeight: 900, color,
        marginTop: metric.icon ? 12 : 0, lineHeight: 1,
      }}>
        {metric.value}
      </div>
      <div style={{
        fontSize: vertical ? 22 : 24, color: props.textColor,
    ...
    <AbsoluteFill style={{ backgroundColor: props.backgroundColor }}>
      ...
      <div style={{
        ...
        <div style={{
          ...
          {metrics.map((m, i) => (
            <MetricCard key={i} index={i} metric={m} color={m.color || COLORS[i % COLORS.length]} props={props} vertical={v} />
          ))}
        </div>
      </div>
    </AbsoluteFill>
...
const StepCard = ({ index, step, color, isActive, props, vertical }) => {
  const anim = useEntrance(props.enableAnimations, staggerDelay(index, 6, 8));
  return (
    <div style={{
    ...
    <AbsoluteFill style={{ backgroundColor: props.backgroundColor }}>
      ...
      <div style={{
        ...
        <div style={{
          ...
          justifyContent: "center", gap: v ? 16 : 12,
        }}>
          {steps.map((step, i) => (
            <StepCard key={i} index={i} step={step} color={COLORS[i % COLORS.length]}
              isActive={activeStep === i} props={props} vertical={v} />
          ))}
        </div>
      </div>
    </AbsoluteFill>
